...
/**
 * Same-origin proxy for OSM raster tiles (Leaflet), ported from the old site's
 * `netlify/functions/osm-tile.mjs` Netlify Function.
...
export async function GET(request: NextRequest) {
  ...
  if (!s || !ALLOW_S.has(s)) {
    return new NextResponse('Invalid tile server', { status: 400 })
  }
