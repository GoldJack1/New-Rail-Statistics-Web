...
const MigrationPage: React.FC = () => {
  ...
  const setMigrationSearchParams = useCallback(
  ...
  const clearMigrationSearchParams = useCallback(
    (replace = false) => {
  ...
  const handleOpenSearchModal = useCallback((matchIndex: number) => {
    ...
    setMigrationSearchParams({ search: '1', matchIndex: String(matchIndex) })
    setState(prev => ({
      ...prev,
  ...
  }, [setMigrationSearchParams, cancelMigrationSearchDebounce])

  const handleCloseSearchModal = useCallback(() => {
    cancelMigrationSearchDebounce()
    if (isSearchPageMode) {
      clearMigrationSearchParams(true)
    }
    setState(prev => ({
      ...prev,
      showSearchModal: false,
  ...
  }, [isSearchPageMode, clearMigrationSearchParams, cancelMigrationSearchDebounce])
                            ...
                            <li key={`${station.id ?? station.crsCode ?? index}-${index}`}>
                              <BUTSharedNativeButton
                                ...
                                onClick={() => {
                                  ...
                                  if (isSearchPageMode) clearMigrationSearchParams(true)
                                }}
                              >
