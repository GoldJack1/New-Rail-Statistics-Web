...
export type StationFetchDetailLevel = 'full' | 'lean'
...
function extractCoordinatesFromFirestoreData(data: Record<string, unknown>): {
  ...
  return { latitude, longitude, extracted }
}

function mapFirestoreDocToStation(
  docId: string,
...
export const fetchStationsFromFirebase = async (
  ...
  options?: { detailLevel?: StationFetchDetailLevel }
): Promise<Station[]> => {
  if (!db) {
  ...
  try {
    ...
    snapshot.forEach((doc) => {
      stations.push(mapFirestoreDocToStation(doc.id, doc.data(), collectionName, detailLevel))
    })
