...
import type { PendingScheduleJobSummary } from '../../../services/firebase'
import { getStationCollectionDisplayLabel, isStationCollectionId } from '../../../constants/stationCollections'
...
import '../../../app/admin/stations/Stations.css'
...
export interface PendingChangesReviewPanelProps {
  ...
  /** Host renders Back / Publish / Schedule (e.g. in `PageTopHeader`); panel exposes live disabled state via this callback. */
  onPageActionBarApi?: (api: PendingReviewPageActionBarApi) => void
...
const PendingChangesReviewPanel: React.FC<PendingChangesReviewPanelProps> = ({
  ...
  const openEditPendingNewStation = useCallback(
    ...
    [router, pathname, pendingChanges]
  )
  ...
  const actionBarApi: PendingReviewPageActionBarApi = useMemo(
    () => ({
      openPublishModal,
      openScheduleModal,
      canMasterPublish,
  ...
  const pageActionsHostedExternally = Boolean(renderPageActionBar || onPageActionBarApi)

  useEffect(() => {
    if (onPageActionBarApi) {
      onPageActionBarApi(actionBarApi)
    }
  }, [onPageActionBarApi, actionBarApi])

  const introSubtitle = pageActionsHostedExternally ? (
  ...
  const introShellClass = pageActionsHostedExternally
