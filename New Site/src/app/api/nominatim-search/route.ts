...
/**
 * Same-origin proxy for Nominatim search (location picker), ported from the old
 * site's `netlify/functions/nominatim-proxy.mjs` Netlify Function.
...
export async function GET(request: NextRequest) {
  ...
  if (!q || q.length > MAX_Q) {
    return NextResponse.json({ error: 'Invalid or missing q' }, { status: 400 })
  }
